...
const LU =()=>{
    ...
    const tofixedValueX = ValueX.map(v => v[0].toFixed(4));
    const formattedValueX = tofixedValueX.join(" , ");
    const [checkValue,setcheckValue] = useState([]);
    const tofixedcheck = checkValue.map(v => v[0].toFixed(4));
    ...
    return (
            <Container>
                <h2 style={{textAlign:"center" ,padding:"20px"}}>LU Decomposition Methods</h2>
                <Form >
                ...
                <h5 style={{padding:"10px"}}>X = </h5>
                    ...
                    <table>
                        ...
                        {ValueX.map((row, rowIndex) => (
                            <tr key={rowIndex}>
                                {row.map((cell, cellIndex) => (
                                <td style={{padding:"10px"}} key={cellIndex}>{cell.toFixed(4)}</td>
                                ))}
                            </tr>
                            ))}
                        </tbody>
